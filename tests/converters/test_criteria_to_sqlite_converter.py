...
@mark.unit_testing
def test_criteria_to_sqlite_converter_escape_like_pattern_value_escapes_wildcards() -> None:
    """
    Test SQLite converter escapes SQL wildcard characters in LIKE pattern values.
    """
    assert CriteriaToSqliteConverter._escape_like_pattern_value(value='100%_off') == '100\\%\\_off'


@mark.unit_testing
def test_criteria_to_sqlite_converter_escapes_like_wildcards_in_contains_query() -> None:
    """
    Test SQLite converter escapes wildcard characters for CONTAINS filters.
    """
...
@mark.unit_testing
def test_criteria_to_sqlite_converter_quote_double_quoted_identifier_escapes_embedded_quotes() -> None:
    """
    Test SQLite converter escapes embedded double quotes in identifiers.
    """
    assert CriteriaToSqliteConverter._quote_double_quoted_identifier(identifier='id" OR 1=1 --') == '"id"" OR 1=1 --"'


@mark.unit_testing
def test_criteria_to_sqlite_converter_convert_escapes_embedded_double_quotes_in_generated_sql() -> None:
    """
...
@mark.unit_testing
def test_criteria_to_sqlite_converter_rejects_deep_criteria_composition() -> None:
    """
    Test SQLite converter rejects criteria trees deeper than the configured maximum.
    """
    nested = Criteria(filters=[Filter(field='name', operator=Operator.EQUAL, value='Doe')])
    for _ in range(CriteriaToSqliteConverter.DEFAULT_MAX_CRITERIA_COMPOSITION_DEPTH + 1):
...
@mark.unit_testing
def test_criteria_to_sqlite_converter_rejects_default_max_page_size() -> None:
    """
    Test SQLite converter applies default pagination maxima when bounds checking is enabled.
    """
    criteria = Criteria(page_size=CriteriaToSqliteConverter.DEFAULT_MAX_PAGE_SIZE + 1, page_number=1)

    with assert_raises(
        expected_exception=PaginationBoundsError,
        match=f'exceeds maximum allowed value <<<{CriteriaToSqliteConverter.DEFAULT_MAX_PAGE_SIZE}>>>',
    ):
        CriteriaToSqliteConverter.convert(
            criteria=criteria,
            table='users',
...
@mark.unit_testing
def test_criteria_to_sqlite_converter_validate_criteria_rejects_unmapped_sql_column() -> None:
    """
    Test SQLite converter rejects mapped SQL columns that are not allowlisted.
    """
    ...
    with assert_raises(
        ...
        CriteriaToSqliteConverter._validate_criteria(
            criteria=criteria,
...
@mark.unit_testing
def test_criteria_to_sqlite_converter_and_criteria_depth_counts_boolean_nodes() -> None:
    """
    Test SQLite converter measures composed criteria depth on boolean nodes only.
    """
    ...
    assert CriteriaToSqliteConverter._criteria_composition_depth(criteria=composed) == 1


@mark.unit_testing
def test_criteria_to_sqlite_converter_rejects_large_operator_allowlist() -> None:
    """
    Test SQLite converter rejects explicit operator allowlists above the configured maximum.
    """
    operators = [Operator.EQUAL] * (CriteriaToSqliteConverter.DEFAULT_MAX_OPERATOR_ALLOWLIST + 1)
...
@mark.unit_testing
def test_criteria_to_sqlite_converter_rejects_large_not_in_list_in_composed_criteria() -> None:
    """
    Test SQLite converter rejects NOT IN lists above the limit inside composed criteria.
    """
    large_values = list(range(CriteriaToSqliteConverter.DEFAULT_MAX_IN_VALUES + 1))
    ...
    with assert_raises(
        ...
        CriteriaToSqliteConverter.convert(
            criteria=criteria,
            table='users',
            max_in_values=CriteriaToSqliteConverter.DEFAULT_MAX_IN_VALUES,
            check_table_injection=False,
            check_column_injection=False,
            check_criteria_injection=False,
            check_operator_injection=False,
...
@mark.unit_testing
def test_criteria_to_sqlite_converter_ignores_non_list_in_values_during_bounds_check() -> None:
    """
    Test SQLite converter skips IN bounds checks when the filter value is not a list or tuple.
    """
    criteria = Criteria(filters=[Filter(field='status', operator=Operator.IN, value='active')])

    CriteriaToSqliteConverter._ensure_criteria_in_list_sizes(
        criteria=criteria,
        limit=CriteriaToSqliteConverter.DEFAULT_MAX_IN_VALUES,
    )
